...
st.set_page_config(page_title="Peak-a-boo Expert Study", page_icon="🔬", layout="wide")

config = load_config()
paths = resolve_paths(config)
bank = load_case_bank(paths.case_bank)
store = StudyStore(paths.database)

st.sidebar.title("Peak-a-boo study")
mode = st.sidebar.radio("Mode", ["Participant study", "Researcher dashboard"])
st.sidebar.caption(f"Study version: {config['study']['study_version']}")
st.sidebar.caption(f"Case bank: {config['study']['case_bank_version']}")
st.sidebar.caption("9 decisions · 3 shared cases · fixed order")
st.sidebar.markdown("**Order:** signal only → separate evidence → AI says peak")
